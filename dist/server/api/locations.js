...
import { eq, desc, asc, like, sql, and, or, isNull } from 'drizzle-orm';
...
export async function GET(request) {
    try {
        ...
        const conditions = [];
        if (search) {
            conditions.push(or(like(locations.name, `%${search}%`), like(locations.code, `%${search}%`), like(locations.city, `%${search}%`), like(locations.state, `%${search}%`)));
        }
        ...
        const countQuery = db.select({ count: sql `count(*)` }).from(locations);
