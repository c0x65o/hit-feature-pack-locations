    ...
    try {
        ...
        if (body.isDefault) {
            await db
                ...
                .set({ isDefault: false })
                .where(and(eq(locationUserMemberships.userKey, body.userKey), eq(locationUserMemberships.isDefault, true)));
        }
        ...
        if (error?.code === '23505' || error?.message?.includes('unique')) {
            return NextResponse.json({ error: 'User is already assigned to this location' }, { status: 409 });
        }
